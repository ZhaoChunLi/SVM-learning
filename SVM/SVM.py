...
def calcWfactors(datamatrix,yofdata,alphas):
	numSamples=datamatrix.shape[0]
	numFetures=datamatrix.shape[1]
	wmatrix= mat(zeros((1,numFetures)))
	for i in range(numSamples):
		wmatrix[0,:] =  wmatrix[0,:] + multiply(yofdata[i] * alphas[i] ,datamatrix[i, :])
	return wmatrix
...
def testSVM(svm, test_x, test_y):
	...
	for i in xrange(numTestSamples):
		...
		#print "right results as follows:"
		print "%d is predicted as %d"%(i,sign(predict))

		if sign(predict) == sign(test_y[i]):
			predictlabels.append(test_y[i])
                        #print "%d is right predicted as %f" %(i, predict)
			matchCount += 1
		else:
			predictlabels.append(-test_y[i])
                        #print "%d is wrong predicted as %f"%(i,predict)
	accuracy = float(matchCount) / numTestSamples
	#predictlabels = mat(predictlabels)
	return accuracy,predictlabels
